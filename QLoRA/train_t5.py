...
# df['input_text'] = "Generate function for CWE: " + df['cwe'].fillna('').astype(str) + " with target: " + df['target'].fillna('').astype(str)
...
# Transform the data into a question-answer format
# Here we're assuming your dataset has columns like 'question' and 'answer'
df['input_text'] = "Question: " + df['cwe'].fillna('').astype(str) + " with target: " + df['target'].fillna('').astype(str)
df['output_text'] = df['func']  # Assuming 'func' contains the answers to the questions
...
def tokenize_function(examples):
    ...
    input_encodings = tokenizer(inputs, truncation=True, padding="max_length", max_length=256)  # Reduced max_length
    label_encodings = tokenizer(labels, truncation=True, padding="max_length", max_length=128)  # Reduced max_length
